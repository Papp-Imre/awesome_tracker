...
A concrete implementation of the `repository` for Flutter apps.

## Getting Started

The code is in `example/repository/lib/`.

The tests can be run from the command line

```
cd flutter_architecture_samples/example/repository
flutter test
```
...
This library implements the `repository`. It uses a File Storage and Web Client (currently a Mock).

This implementation tries first to load the tasks from storage, then falls back to web if none are found. It persists changes to both the file system and the web service.
